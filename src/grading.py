...
def grading(sl, phi, z, intv, porosity, ni):
    ...
    # initiate array with size of particles hitting bed
    hitsize = zeros(shape=(numrow_sl, numcol_sl))
    # initiate array with concentration of particles hitting bed
    hitload = zeros(shape=(numrow_sl, numcol_sl))
    # initiate array with height of particles hitting bed
    hitheight = zeros(shape=(numrow_sl, numcol_sl))
    if((len(z) != numrow_sl) or (len(phi) != numcol_sl)):
        ...
        setvel = 0.01*tubesetvel(phi, rhos=2.65, T=21.6, S=0)
...
# **********************************find size of sediment hitting the bed****************************************
        ...
        for i in range(len(phiind)):
            # keep track of order sediment size hitting the bed
            hitsize[i] = phi[phiind[i]]
# ***********************************find elevations that sediment started from**********************************
        zind = stime[:, 1]
        # print ind
        hitheight = zeros(len(zind))
        for i in range(len(zind)):
            # keep track of order of height sediment came from
            hitheight[i] = z[zind[i]]
        hitload = zeros(len(zind))
        for i in range(len(zind)):
            hitload[i] = sl[zind[i], phiind[i]]
        # cumulative thickness of deposit
        thickness = cumsum(hitload/(1-porosity))
        # print sum(thickness)
# ********************************************determine interval breaks*******************************************
        ...
        if(len(intv) == 1):  # calculate interval boundaries for set intervals
            ...
            while(i < len(intbind)+1):
                ...
                i = i+1
            # add break at first and last thickness value
            intbind[i] = len(thickness)
...
# **************************************calculate stats for each interval*****************************************
        ...
        for j in range(0, len(indthick2)-1):
            ...
            p[j, :] = phi
            s[j, :] = sload[j]
            # weight percent in each phi interval
            wpc[j, :] = 100*sload[j]/(sum(sload[j]))
...
# This figure is desinged to show the relationship between location and first hit grain size
        ...
        ax1.plot(m1, midint, '-')
        savefig('/Users/tanghui/Dropbox/project1/project1/model01/model1_2/figure/m1/' +
                fname1+' Sample%i.png' % ni, dpi=100)
...
# This figure is designed to show the relationship between location and standard deviation
        ...
        savefig('/Users/tanghui/Dropbox/project1/project1/model01/model1_2/figure/stdev/' +
                fname2+' Sample%i.png' % ni, dpi=100)
...
# This figure is designed to show the relationship between location and mean
        ...
        savefig('/Users/tanghui/Dropbox/project1/project1/model01/model1_2/figure/fwmedian/' +
                fname3+' Sample%i.png' % ni, dpi=100)
...
# This figure is designed to show the relationship between location and sorting
        ...
        savefig('/Users/tanghui/Dropbox/project1/project1/model01/model1_2/figure/fwsorting/' +
                fname4+' Sample%i.png' % ni, dpi=100)
...
# This figure is designed to show the relationship between location and skewness
        ...
        savefig('/Users/tanghui/Dropbox/project1/project1/model01/model1_2/figure/fwskew/' +
                fname5+' Sample%i.png' % ni, dpi=100)
...
# This figure is designed to show the reltaionship between location and kurtosis
        ...
        savefig('/Users/tanghui/Dropbox/project1/project1/model01/model1_2/figure/fwkurt/' +
                fname6+' Sample%i.png' % ni, dpi=100)
...
# This figures is designed to show the relationship between thickness and time sediment hits bottom
        ...
        savefig('/Users/tanghui/Dropbox/project1/project1/model01/model1_2/figure/hittime/' +
                fname7+' Sample%i.png' % ni, dpi=100)
