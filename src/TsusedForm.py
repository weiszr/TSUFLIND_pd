    ...
    with open('parameter_P14abc.txt', 'r') as f:
        for line in f:
            try:
                ...
                v = s[1]  # remove '\n'
                if os.name != 'nt':
                    v = v[0:len(v)]  # remove '\r' for Unix-system
                if n == 'porosity':
                    porosity = float(v)
...
# ================================Calculate Settling velocity======================================
        ...
        setvel = tubesetvel(phi, Rhos, wtemp, sal)
...
# =================================Calculate Settling Time=========================================
        # Settling time for each grain size
        settlingtime = zeros(shape=(numrow_sl, numcol_setvel))
        # store for sorting based on settling time
...
# =========================find elevations that sediment started from=============================
        ...
        for i in range(len(zind)):
            ...
            hitheight[i] = z[int(zind[i])]
        # initial array with concentration of particles hitting bed
...
# =================================Calculate grainsize distribution================================
        ...
        # Pecentage for each grain size
        Fr = zeros(shape=(len(indthick2)-1, len(phi)))
...
# ========================Plot grainsize distribution for each interval============================
        ...
        for i in range(0, int(len(intv))):
            ...
            ax1.set_ylabel(
                'The percentage of different sediment grains size(%)')
