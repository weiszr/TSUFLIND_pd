...
# ==============================Caculate critical velocity and critical shear stress=====================
    # Critical shear velocity for initiation of sediment transport
    Ucrit = zeros(nc)
    tcrit = zeros(nc)  # Critical shear stress (N/m^2)
    C = zeros(nc)  # Original concentration for each grain size
    for i in range(nc):
        # caculate critical shear velocity for initiation of sediment transport
        Ucrit[i] = UstarCrit(se[i], KinVisc(wtemp), Rhos/rho)
        tcrit[i] = rho*Ucrit[i]**2  # Caculate critical shear stress (N/m^2)
    # bottom stress(?)
    taub = (Vi*vk/(log(30*H/(2**(-1)-2**(-9))-(1-(H*(2**(-1)-2**(-9)))))))**2*rho
    taustar = taub/tcrit
