...
def Tsusedmod2(p_file_l,name):
        ...
        for line in f:
            try:
                ...
                # data
                    ...
                    num = int(v)
                elif n == 'Number_grainsize':
                    nclass = int(v)
                elif n == 'data_dimension':
                    l = int(v)
    ...
    # normalize size fractions to avoid rounding errors in sed. analysis causing problems
    fr = fr/sum(fr)
    # susp sed size distribution wanted, keep this for comparison with model results
    ssfrwanted = fr
    # initiate ss converge to 1 for all size classes (does  converge)
    ssconverge = ones(shape=(nclass, 1))
    for i in range(len(fr)):
        if(fr[i] != 0):
            # reset ss converge values to 0 (does not converge) for all size classes with sediments
            ssconverge[i] = 0
    # set percent difference in size distribution allowed, same value for all sizes
    sizeconverge = sizeconvfactor*ones(shape=(nclass, 1))
    # convert grain size to mm for settling velocity calculation using Dietrich's formulae
    D = phi2mm(phi)
    ...
    # set sediment density, same value for all sizes
    rhos = Rhos*ones(shape=(nclass, 1))
# ==========================================================Calculate water parameter=============================================================
...
# ============================================================calculate zo=======================================================================
    ...
    ustrcAdjFac = 0.01
    # need critical shear velocity for mean grain size to calculate bed roughness
    UcritDmn = UstarCrit(Dmn, kinvisc, 2650/rhow)
    if(zotot != 1):
        # zo total (Nickaradse bed roughness and Wiberg/Smith bed roughness from saltation,
        zotot = zoN+zoWS(Dmn, ustrc, UcritDmn, rhow)
# ===============================================calculate shear velocity needed to suspend deposit===============================================
    ...
    offa = zeros(nit)
    # inititate array to track how  u*c changes with iteration                                            #ref. Wiberg and Rubin, 1989 JGR
    ustrca = zeros(nit)
    cconverge = 0
    for iteration in range(int(nit)):
        ...
        for i in range(len(S)):
            # reference concentration (volume conc; m3/m3),ref. elevation is zo, not 2 grain diameters
            Ca[i] = g0*Cb*fr[i]*S[i]/(1+g0*S[i])
...
# *********************************calculate the suspended-sediment profile and sediment load for each class****************
        ...
        for i in range(nclass):
            ...
            for j in range(int(nz-2)):
                # used to create deposit by sediment settling
                sl[j][i] = 0.5*((C[j][i]+C[j+1][i])*diff1[j])
                # used to create deposit by sediment settling
                sl1[j][i] = 0.5*((C[j][i]+C[j+1][i])*diff1[j])
...
# ************************************************** set cconverge to 0*****************************************************
        ...
        # change ustrc based on how far off suspended load is from desired value
        ustrc = ustrc*(1-ustrcAdjFac*off)
        if(zotot != 1):
            # recalculate bed roughness
            zotot = zoN+zoWS(Dmn, ustrc, UcritDmn, rhow)
# *******check for convergence for size of suspended sediment and adjust bed sediment distribution every diststep***********
        ...
        if(doss == iteration):  # redo distribution every diststep steps
            ...
            ssfra[int(doss/diststep), :] = ssfr
            fra[int(doss/diststep), :] = fr.reshape(1, len(fr))
            ...
            for k in range(nclass):
                if(ssfrwanted[k] != 0):  # only adjust for classes with material in them
                    ...
                    # change bed grain size distribution based on suspended load distribution
                    ssoffa[int(doss/diststep), k] = ssoff
                    fr[k] = fr[k]*(1-0.01*ssoff)
                    # check for convergence for each size class
                    if (abs(ssoff)*100 < sizeconverge[k]):
    ...
    # speed calculated using eddy viscosity profile integration
    spd[:] = cumsum(diff1*ustrc*ustrc/gelfK(zmid, h, ustrc))
    if(iteration == nit):
