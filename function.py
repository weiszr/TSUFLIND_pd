...
def dietrichWs(T, D, rhos, rhow, Cs, csf, P):
    ...
    lu = KinVisc(T)
    # convert kinematic viscosity in m**2/s to dynamic viscosity in poise
    mu = KinVisc(T)*rhow*10000
    ...
    # eq. 6, Dietrich '82 (D82), converting size from mm to cm
    dstar = (rhos-rhof)*g*(D/10)**3/(rhof*nu**2)
    dstarl = log10(dstar)
    r1 = -3.76715+1.92944*dstarl-0.09815*dstarl**2-0.00575*dstarl**3+0.00056 * \
        dstarl**4  # fitted equation for size and density effects, eq. 9 D82
    omc = 1.0-csf
    r2 = log10(1-omc/0.85)-omc**2.3*tanh(dstarl-4.6)+0.3*(0.5-csf) * \
        omc**2*(dstarl-4.6)  # fitted equation for shape effects. eq. 16 D82
    # fitted equation for roundness effects, eq. 18 D82
    r3 = (0.65-(csf/2.83)*tanh(dstarl-4.6))**(1+(3.5-P)/2.5)
...
def sw_dens0(S, T):
    ...
    if(nargin != 2):
        ...
        d0 = 4.8314e-4
        dens1 = sw_smow(T)
        dens = dens1+(b0+(b1+(b2+(b3+b4*T)*T)*T)*T)*S + \
...
def tubesetvel(phi, rhos, T, S):
    ...
    # print rhof
    # dynamic viscosity, converted to poise (dyne-sec/cm^2)
    dynvisc = 10*kv*rhof
    # print dynvisc
    ...
    r = (1.0/(2.0**phi))/20.0
    term = sqrt((9.0*dynvisc**2)+(g*r**2*rhof *
                                  (rhos-rhof)*(0.015476+(0.19841*r))))
    ...
    if(type(kinvisc) == 'float'):
        ...
        for i in range(mK):
            for j in range(nK):
                ...
                if (sstar[i][j] >= 0.8):
                    shieldscrit = 0.095 * \
                        sstar[i][j]**(2.0/3.0) + 0.056 * \
                        (1-exp((-sstar[i][j]**(3.0/4.0))/20.0))
                else:
                    shieldscrit = 0.1*sstar[i][j]**(-2.0/7.0)
                # critical shear velocity in m/s
                ustarcrit[i][j] = sqrt((s-1)*g*d)*sqrt(shieldscrit)
        ...
        if (sstar >= 0.8):
            shieldscrit = 0.095*sstar**(-2.0/3.0) + \
                0.056*(1-exp((-sstar**(3.0/4.0))/20.0))
        else:
            shieldscrit = 0.1*sstar**(-2.0/7.0)
        # critical shear velocity in m/s
        ustarcrit = sqrt((s-1)*g*d)*sqrt(shieldscrit)
    return (ustarcrit)
# ===============================================function zoD=================================================
...
def zosD(T, D, ustarc, rhow, s):
    ...
    for i in range(mU):
        for j in range(nU):
            ...
            # Dietrich 1982 from Wiberg and Rubin, 1989 equation 3
            zosD[i][j] = alpha*D*(0.6*Tstar[i][j]/(1+0.2*Tstar[i][j]))
    return zosD
# ===================================================function zoWS===========================================
...
def zoWS(D, ustarc, ustarcrit, rhow):
    gammaWS = 0.056
    a1 = 0.68
    # equation 6, D converted to cm, Wiberg and Rubin, 1989
    a2 = 0.0204*(log(D*100))**2+0.0220*log(D*100)+0.0709
    taub = rhow*ustarc**2
