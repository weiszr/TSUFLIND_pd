...
def readCSV(name, separator=';'):
    data = zeros([1, 2])
    start = True  # initialize data array
    with open(name, 'rU') as f:
        reader = csv.reader(f)  # read CSV file
        for row in reader:  # read each line and store them into a n by 2 array
            try:
                rs = row[0].split(separator)
                d1 = float(rs[0])
                d2 = float(rs[1])
            except IndexError:
                continue  # if line is not delimited by separator, continue
            except ValueError:
                continue  # if not numbers, continue
            if (d1 != "\x00"):
                if not start:
                    data = append(data, [[d1, d2]], axis=0)
                else:
                    start = False
                    data[0] = [d1, d2]
    return data
...
def readCSV1(name, separator=';'):
    ...
    with open(name, 'rU') as f:
        ...
        for row in reader:  # read each line and store them into a n by 2 array
            ...
            if (d1 != "\x00"):
                if not start:
                    data = append(data, [[d1, d2, d3, d4, d5]], axis=0)
                else:
                    start = False
...
def readCSV2(name, separator=';'):
    ...
    with open(name, 'rU') as f:
        ...
        for row in reader:  # read each line and store them into a n by 2 array
            ...
            if (d1 != "\x00"):
                if not start:
                    data = append(data, [[d1, d2, d3]], axis=0)
                else:
                    start = False
                    data[0] = [d1, d2, d3]
    return data
